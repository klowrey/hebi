...
using BlackBoxOptim # has to come after `using Parameters b/c of conflict`
...
function gethebivars()
    return MultiShape(
        x8_damping = ScalarShape(Float64),
...
function filesysid(
    ...
    test::HEBIPickup,
    ctrls,
    modelvars = gethebivars();
    optm = :NM,
    batch = 1:size(refstate, 2),
    horizon = length(batch),
    velscale = 1,
)
    N = length(modelvars)
    osp = obsspace(test)
    ...
    function opt(P, env = test, Y = refstate, batch = batch)
        ...
        vid = nq .+ idx
        return (
            mse(Y[idx, batch], roll.obses[idx, :]) +
            velscale * mse(Y[vid, batch], roll.obses[vid, :])
        ) / 2.0
    end
    ...
    elseif optm == :PS
        ...
        result = optimize(
            x -> t_opt(x; nthreads = 1),
            initP,
            ParticleSwarm(lower = lower, upper = upper, n_particles = 18),
            options,
        ) # doesn't work?
    elseif optm == :BBO
        ...
        bbopt = bbsetup(
            p -> opt(p, test);
            Method = :xnes,
            SearchRange = (0.0001, 5.0),
            NumDimensions = length(modelvars),
            MaxFuncEvals = 1000,
        )#, Workers = workers())
...
function getsincontrols(env::HEBIPickup, T = 2000)
    reset!(env)
...
e1 = HEBIPickup()
e2 = HEBIPickup()
a = getsincontrols(e1, 2000)
result = testsysid(e1, e2, a; optm = :LBFGS)
